...
void TestWorldStepsAndExportsStates() {
    ...
    Require(states.front().entity_id == 1, "第一个 entity 应该是 player cube");
    Require(states.front().position.x > 0.05, "player cube 应该沿 X 方向移动");
    Require(states.front().position.y > 0.2, "player cube 不应该掉穿地面");
}
